...
import utility
from models.sys_role import Sys_role
...
class GetRole(Resource):
    ...
    def get():
        try:
            with db_session:
                data = select(row for row in Sys_role)
    ...
    def post():
        try:
            with db_session:
                ...
                c = Sys_role(sysrole_kode=sysrole_kode, sysrole_nama=sysrole_nama)
                commit()
...
class UpdateRole(Resource):
    ...
    def put(sysrole_kode):
        try:
            with db_session:
                sysrole_nama = request.form['sysrole_nama']
                sys_role = Sys_role[sysrole_kode]
